...
При получении SIGHUP (сигнал, который получает программа и при закрытии терминала) процесс демонизируется: выполняется fork, родительский процесс завершается, дочерний процесс вызывает setsid() и становится лидером новой сессии. Затем выполняется второй fork(), после которого первый дочерний процесс завершается, а итоговый процесс-демон уже не является лидером сессии и не может повторно получить управляющий терминал. После этого stdout/stderr перенаправляются в файл.
Программа продолжает работу в фоновом режиме.
